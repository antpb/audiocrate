//
//  GrainDelayEngine.h (class name AmpDelayEngine kept — per-target copy convention)
//  homecrate grain
//
...
private:
    ...
    // Fractional (linear-interp) read at writePos - offset. Indices are
    // clamped to the vector's live size: OS 26's hardened libc++ aborts the
    // extension on any vector[] overrun, and a read that races a re-init
    // (mBufferSize/mWritePos in flux) must degrade to a wrong sample, not
    // kill the render server (Vision Pro .ips 2026-07-23, monitor start).
