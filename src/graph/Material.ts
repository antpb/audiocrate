...
import { MaterialNotBoundError } from '../errors';
...
/**
 * The object a Material's `graph` builder receives. Arbitrary names
 ...
 */
export type MaterialGraphContext = Record<string, ASLValue> & {
...
export type MaterialGraphBuilder = (ctx: MaterialGraphContext) => ASLValue;

export type { VoiceStealingPolicy, VoiceSnapshot } from '../voices/allocate';

/**
 * How this Material's output should land on another Material's param jack.
...
/**
 * Where a Material's second audio input comes from. Declared on the Material
 * and resolved by whoever builds the real graph (`ScenePlayback`), because a
 * Material is authored long before it knows what scene it will live in, and
...
export interface MaterialOptions {
  name: string;
  /**
   * Registry key for the plugin that owns this Material (`registry/`).
  ...
  /**
   * Output channels to evaluate. Left unset, a Material that reads live
   * audio runs once per channel (a real stereo insert) and one that does not
   * runs once and is mirrored. Set `1` on a Material whose output is a
...
export class Material {
  ...
  /** Registry key for the plugin that created this Material. See `registry/`. */
  ...
  /**
   * Where this Material came from in a hosted project: that project's own
   ...
   * voice this Material is bound to: crate's own `track.id` and the dense
  ...
  /**
   * Decoded files this Material's preset referenced, keyed by whatever the
   ...
   * third-party Material might need, so it does not try.
   */
  ...
  private readonly graphBuilder: MaterialGraphBuilder;
  ...
  constructor(options: MaterialOptions) {
    ...
    if (this.polyphony < 1) {
      throw new RangeError(`Material "${this.name}": polyphony must be >= 1`);
    }
    ...
    for (const key of this.automatable) {
      if (!(key in this.params)) {
        throw new RangeError(`Material "${this.name}": automatable references unknown param "${key}"`);
      }
    }
    ...
    if (!(output instanceof ASLValue)) {
      throw new TypeError(`Material "${this.name}": graph builder must return an ASLValue.`);
    }
    this.graph = { inputs: Object.keys(this.params), output: output.node, channels: options.channels };
  }

  /**
   * Live audio inputs this Material's graph reads, sorted, `input` included
   * when it reads one. A host wiring a sidechain needs this to know the
   * Material wants one at all; the renderer turns it into worklet input
  ...
  /**
   * Feeds one of this Material's aux inputs from somewhere else in the
  ...
  setAudioSource(port: string, source: AudioSourceRef | null): void {
    if (!this.auxAudioInputs.includes(port)) {
      throw new RangeError(
        `Material "${this.name}" has no audio input "${port}" (has: ${
  ...
  getParam(name: string): number {
    if (!(name in this.params)) {
      throw new RangeError(`Material "${this.name}" has no param "${name}"`);
    }
  ...
  setParam(name: string, value: number): void {
    ...
    if (value < descriptor.min || value > descriptor.max) {
      throw new RangeError(
        `Material "${this.name}".${name} must be within [${descriptor.min}, ${descriptor.max}], got ${value}`,
      );
    }
  ...
  setOption(name: string, option: string): void {
    ...
    if (descriptor.kind !== 'enum') {
      throw new TypeError(`Material "${this.name}".${name} is a ${descriptor.kind} param, not an enum`);
    }
  ...
  noteOn(note: number, options?: { velocity?: number } & Record<string, number>): void {
    if (!this.pool) throw new MaterialNotBoundError(this.name);
  ...
  /**
   * A new Material of the same type at the same param values. Two editor
...
export class MaterialChain {
  ...
  remove(material: Material): boolean {
  ...
  get list(): readonly Material[] {
    return this.items;
  }

  [Symbol.iterator](): Iterator<Material> {
